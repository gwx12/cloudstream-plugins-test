...
class KisskhProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        Log.d("KisskhProvider", "Starte loadLinks für Episode: ${loadData.epsId} Titel: ${loadData.title}")
        val kkey = app.get("$KisskhAPI${loadData.epsId}&version=2.8.10", timeout = 10000).parsedSafe<Key>()?.key ?:""
        Log.d("KisskhProvider", "KKey erhalten: $kkey für Episode: ${loadData.epsId}")
        val videoUrl = "$mainUrl/api/DramaList/Episode/${loadData.epsId}.png?err=false&ts=&time=&kkey=$kkey"
        Log.d("KisskhProvider", "Hole Video-Source: $videoUrl")
        ...
        Log.d("KisskhProvider", "Video-Response Header: ${videoResponse.headers}")
        Log.d("KisskhProvider", "Video-Response Cookies: ${videoResponse.cookies}")
        videoResponse.parsedSafe<Sources>()?.let { source ->
            listOf(source.video, source.thirdParty).amap { link ->
                safeApiCall {
                    Log.d("KisskhProvider", "Verarbeite Link: $link")
                    if (link?.contains(".m3u8") == true) {
                        Log.d("KisskhProvider", "M3U8-Link erkannt: $link")
                        M3u8Helper.generateM3u8(
                            ...
                            Log.d("KisskhProvider", "M3U8-Stream gefunden: ${it.url} | Header: ${it.headers}")
                            callback(it)
                        }
                    } else if (link?.contains("mp4") == true) {
                        Log.d("KisskhProvider", "MP4-Link erkannt: $link")
                        callback.invoke(
                            newExtractorLink(
                                this.name,
                                this.name,
                                ...
                                Log.d("KisskhProvider", "MP4-ExtractorLink erstellt: $url | Referer: $referer | Quality: $quality")
                            }
                        )
                    } else {
                        val extractorUrl = link?.substringBefore("=http") ?: return@safeApiCall
                        Log.d("KisskhProvider", "Unbekannter Link, versuche loadExtractor: $extractorUrl")
                        loadExtractor(
                            extractorUrl,
                            "$mainUrl/",
                            subtitleCallback,
        ...
        Log.d("KisskhProvider", "Subtitle-KKey erhalten: $kkey1 für Episode: ${loadData.epsId}")
        val subUrl = "$mainUrl/api/Sub/${loadData.epsId}?kkey=$kkey1"
        Log.d("KisskhProvider", "Hole Subtitles: $subUrl")
        val subResponse = app.get(subUrl)
        Log.d("KisskhProvider", "Subtitle-Response Header: ${subResponse.headers}")
        Log.d("KisskhProvider", "Subtitle-Response Cookies: ${subResponse.cookies}")
        subResponse.text.let { res ->
            tryParseJson<List<Subtitle>>(res)?.map { sub ->
                Log.d("KisskhProvider", "Subtitle gefunden: ${sub.src} | Label: ${sub.label}")
